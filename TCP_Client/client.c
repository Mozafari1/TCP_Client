...
#pragma warning(disable : 4996) // Deaktiverer link feilen
...
int main(int argc, char *argv[])
{
	...
	client.sin_addr.s_addr = inet_addr("ENTER YOUR IP");
	client.sin_family = AF_INET;
	client.sin_port = htons(ENTER YOUR PORT);
	...
	server_reply[0] = 0x1;// arryet ser slik ut [0x01]
	// lager en meldings størrelse i short med 7 plasser
	unsigned M_lengde = htons((short)7);
	// kopierer en block med data fra en kilde address til en plassering address.
	memcpy(server_reply+1, &M_lengde,2);// arryet ser slik ut [0x01,0x00, 0x07]
	memcpy(server_reply+4, message,6); // arryet ser slik ut [0x01,0x00,0x07,0x06,0,1,2,4,5,6]
